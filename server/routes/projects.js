...
router.post('/', (req, res) => {
  const db = getDb();
  const { project_number, description, currency, delivery_level } = req.body;
  ...
  const create = db.transaction(() => {
    const result = db.prepare(
      `INSERT INTO projects (project_number, description, currency, delivery_level)
       VALUES (?, ?, ?, ?)`
    ).run(project_number, description, currency || 'USD', delivery_level || 1);
    ...
    db.prepare(`INSERT INTO project_func_phase_pct (project_id, prep, fts, design, build, sit_uat, dep, hyp, architect_pct)
      VALUES (?, 0.15, 0.15, 0.10, 0.30, 0.24, 0.06, 0.10, 0.10)`).run(pid);
...
router.put('/:id', (req, res) => {
  ...
  ).run(project_number, description, currency, delivery_level, req.params.id);
  res.json({ ok: true });
});
